...
account that the problem also needs to be compiled before it can be solved,
...
package \cc \citep{ccSolve}, provides an interface to problems written in 
...
It is meant to speed up the solution of intial value (IVP) and boundary value problems (BVP) for 
...
The idea is to formulate the problem as text strings that are either \proglang{Fortran}, 
\proglang{F95} or \proglang{C} code, but without specifying the headers and 
declaration section of the code.
\cc functions are then used to complete these codes, by adding the required 
declarations and parts of the codes that perform technical manipulations. 
The functions then compile the code, and load the DLL or shared object. 
This resulting object can then be used as argument in the 
associated solver.
...
on the R-package \pkg{inline} \citep{inline}. 
...
@
The \cc function \code{ccuniroot} extends the original C-code to accept problems
specified in compiled code (\footnote{I use the simple uniroot function from R2.12, as it is simpler})
...
If we print the \proglan{F95} code, the wrapper written by \code{compile.uniroot}
is clear:
<<>>=
code(croot.f95)
code(croot.C)
@
The returned object can be queried using \code{ccfunc}:
<<>>= 
ccfunc(croot.f95, x = 1)
@

\subsection{multiple roots}
The R-function that makes the compiled code for multidimensional 
...
so the same argument is used during compilation
...
is added to the code, that allows the solver to put the parameter values, as passed
...
We solve the problem, using function \code{multiroot.1D}, which assumes a banded Jacobian.
...
\pkg{minpack.lm} \citep{minpack.lm}, so this was used for extension to compiled 
code instead. 
Apart from the \language{Fortran} versus R implementation, the method in the latter 
package by default scales the parameters, which may make it slightly more efficient.
...
as present in \code{data.frame} \code{DNase1}, are \code{conc} and \code{density}.

Compiled code cannot work with the formula notation, so the residuals are specified
instead, and put in vector \code{f}. 
To make the compiled code readable, the problem is compiled such that the 
...
during the application. This is done by calling the added subroutine \code{initdat},
...
\proglang{F95} works with vectors, so this gives the most simple codes:
<<>>=
...
code(cfm.f95)
@
For \proglang{C} the residuals need to be calculated using a loop:
<<>>=
...
the sum of squared residuals is minimised; the function to minimise, in \proglang{R} and 
\proglang{F95}:
<<>>=
Opt.R <- function(p) {
  with (DNase1[,-1],
